...
M4-A 原型已通过仓库级、APK 级与设备级验收。插件现在从官方 OpenCC 1.4.2 源码构建
`libopencc_jni.so`，正式 APK 不再打包旧 `android-opencc` 的原生库与资源。M4-B 的差分语料、
异常路径和 16 KB 静态门禁也已落地；性能报告、完整设备矩阵与真实 16 KB 页设备仍待完成，
因此当前尚不进入 v1.2.0 发布。
...
- 在 arm64/arm32 新后端设备和完整 API 24–36 矩阵重放增强后的设备测试。
- 记录首次加载、冷/热转换、批量转换、峰值内存和可比的 release APK 体积。
- 在 `PAGE_SIZE=16384` 的 Android 15+ 真机或模拟器完成安装与 14 类型运行验收；目前设备页大小
  均为 4096，不能用 ELF/ZIP 静态门禁替代真实运行结果。
...
主要固定成本是 1,237,703 B 的官方纯文本资源 ZIP；M4-B 性能测试完成前不提前改为按 ABI
生成 `.ocd2`，避免用体积优化引入不可验证的跨架构词典格式问题。
