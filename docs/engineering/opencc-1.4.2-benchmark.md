...
本报告数据在 M4-B 迁移窗口由当时的 `OpenccPerformanceBenchmarkTest` 和
`scripts/benchmark/run_opencc_benchmark.py` 共同采集。宿主运行器会拒绝重置已安装的插件，
除非显式传入 `--allow-package-reset`；当时每个引擎均经过独立的卸载/安装周期，并使用两个不同
PID。M4-C 删除旧 AAR 后，当前测试与运行器只测量官方后端；下表旧引擎数据作为已完成迁移的
历史证据保留，不再要求未来测试包重新携带旧实现：
...
ASCII、emoji 和 U+20000 非 BMP 字符。采集时两引擎的输出都由测试断言为相同的预期繁体结果。
时间数据来自当时的 debug 目标和 instrumentation 测试，只用于同设备、同 APK、同负载的迁移
判断，不是跨设备性能承诺，也不设置容易受模拟器负载影响的 CI 时间阈值。
...
M4-B 采集时服务销毁会调用 `nativeClearCache`；M5-A 引入独立 App 入口后，当前实现改由应用进程级
协调器统一持有缓存，并在进程结束时回收，避免服务销毁关闭 UI 正在使用的引擎。两种生命周期下
并发调用都串行进入同一转换器，不会为每个 Binder 或 UI 请求复制一套字典；本表历史测量值不变。
...
资源 ZIP 为五个 APK 的共同固定成本；universal 还同时携带四 ABI 静态链接库。本表对应
2026-09-01 已通过源码标签、GitHub Release 与插件索引交叉核验的 v1.2.0 正式发布产物。
