...
- [x] (CI/在线复验) draft 控制器落地后，[Build integrity run 33737709352](https://github.com/SuperMonster003/AutoJs6-Plugin-OpenCC/actions/runs/33737709352) 在精确提交 `b41d0286b071ba6160ea83c5dba5c2da11a5b474` 再次通过构建/API 24/arm64/x86_64 4 KB/x86_64 16 KB 五项且零 annotation；[candidate run 33738494609](https://github.com/SuperMonster003/AutoJs6-Plugin-OpenCC/actions/runs/33738494609) 随后通过同 SHA 的签名、深度 APK、清单与清理门禁，唯一 artifact `opencc-signed-candidate-v1.3.0-build20-b41d0286b071`（ID `9886881929`，9,749,717 B，归档 SHA-256 `d54e4bef6d114ab30185dd9f33640c9b49cedd601d15f3f4b8048c16b96b5707`）。API 35 无头模拟器曾证明 `Activity.hasWindowFocus()` 不是可靠的剪贴板权限代理，仪器测试现以前台恢复后的真实 Paste 可见结果和 ClipboardManager 回读失败关闭，并在 API 35/API 28 真机及完整 Actions 矩阵复验通过。
...
- [x] (发布/在线验收/draft 锁定) [run 33739003669](https://github.com/SuperMonster003/AutoJs6-Plugin-OpenCC/actions/runs/33739003669) 在同一精确 SHA 和 `pr-only` 模式下真实调用 `operation=draft`：仅无 Environment/secret/写权限的锁定报告成功，凭据、候选、显式 Build/Markdown dispatch 和 draft 写入四个 job 全部 skipped，artifact 与 annotation 均为 0，且未新增 Environment deployment。前后 Release/tag/Latest 均不变，索引仍为 `3b9b47cf4acd306ab2de63638e1aa761c82c28ad` 且无新 run，仓库变量仍为 `pr-only`；这只验收锁定路径，不代表已放行实际草稿写入。
