...
当前检查点 (2026-08-31): M4-A 已完成; M4-B 的差分语料、异常路径和 16 KB 静态门禁已完成,
完整性能/体积报告及真实 16 KB 设备仍待验收; M4-D 的只读监视已提前启用, 自动升级 PR 仍按顺序等待.
...
- [ ] (测试) 记录旧/新引擎的首次加载, 冷转换, 热转换, 批量转换, 峰值内存和五种 APK 体积; 五种 minified release APK 的初步体积基线已归档, 性能/内存数据仍待采集. 纯文本资源首次解析如不满足体验要求, 再评估由 CI 按目标架构生成并验证 `.ocd2`, 不预先引入跨架构词典构建复杂度.
...
- [ ] (测试) 在现有 API 24–36 四 ABI 设备矩阵外, 增加 `PAGE_SIZE=16384` 的 Android 15+ 环境, 完成安装, 插件发现, Binder 往返和 14 类型转换; 只有最终 JNI, APK 打包和 16 KB 设备三层均通过后才声明支持.

M4-B 验收条件: 差分结果均可解释并有审阅记录; 新后端在现有四 ABI/API 矩阵无行为回归; 性能和体积报告已归档; 16 KB ELF, ZIP 对齐和真实运行环境全部通过.
...
- [ ] (CI/依赖) 更新脚本必须校验标签提交, GitHub 资源 digest, ZIP 内 manifest 提交和逐文件摘要, 同步更新子模块指针及锁定文件, 然后生成包含上游 Release 摘要和版本差异的 PR.
- [ ] (CI/测试) 升级 PR 强制执行四 ABI 构建, 14 类型与差分语料, Binder 设备测试, 16 KB/RELRO/ZIP 对齐, APK 体积, 许可证及资源清单校验; 任一门禁失败即不得合并.
- [ ] (发布) 上游升级 PR 保持人工合并和人工发版; 紧急修复可手动触发同一流程, 但不得绕过版本锁定与输出差异审阅.

M4-D 验收条件: 使用一个模拟的新上游版本或当前版本重放完整检查, 能稳定地产生无漂移更新或明确的无更新结果; 清洁 CI 环境不依赖 Maven Local, 开发机缓存或 `T:\\...` 路径.
