...
- `提示` 同一個 APK 現在既可在 Android 7.0+ 上作為完全離線的獨立應用程式使用, 亦繼續作為原有 AutoJs6 插件使用; 只有插件路徑需要 AutoJs6
- `新增` 新增桌面啟動入口和覆蓋全部 14 種 OpenCC 轉換類型的完全離線編輯器, 提供轉換, 取消, 清空, 貼上, 交換, 複製與分享操作, 支援背景處理長文本以及旋轉/程序狀態恢復
- `新增` 新增 10 語言獨立介面, 支援日/夜主題, RTL, 大字體, TalkBack 語義與焦點次序, 硬件鍵盤快捷鍵, 可獨立捲動/選取的編輯區域及手機, 平板和分割畫面響應式佈局
