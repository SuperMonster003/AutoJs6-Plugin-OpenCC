...
- `提示` 同一个 APK 现在既可在 Android 7.0+ 上作为完全离线的独立应用使用, 也继续作为原有 AutoJs6 插件使用; 只有插件路径需要 AutoJs6
- `新增` 新增桌面启动入口和覆盖全部 14 种 OpenCC 转换类型的完全离线编辑器, 提供转换, 取消, 清空, 粘贴, 交换, 复制与分享操作, 支持后台处理长文本以及旋转/进程状态恢复
- `新增` 新增 10 语言独立界面, 支持日/夜主题, RTL, 大字体, TalkBack 语义与焦点顺序, 硬件键盘快捷键, 可独立滚动/选择的编辑区域及手机, 平板和分屏响应式布局
